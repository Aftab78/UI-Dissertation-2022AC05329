...
from models import CNN_Model,RNN_Model,LSTM_Model,PROPHET_Model,Lagllama_Model
from missing_handler import mean_case,median_case,mode_case,forward_fill_case,backward_fill_case
from missing_handler import linear_interpolation_case,polynomial_interpolation_case,k_nearest_case,choose_best_method
...
with menu[3]:
    if st.session_state['cleaned_data'] is not None:
        ...
        #st.write("time_freq = ",time_freq)
        if(time_freq=='Y'):
            data_period=1
        elif(time_freq=='Q'):
            data_period=4
        elif(time_freq=='MS' or time_freq=='M'):
            data_period=12        
        elif(time_freq=='W'):
            data_period=52
        elif(time_freq=='D'):
        ...
        for i in range(len(forecast2)): 
            #print("***************",level[i],trend[i],season[i])           
            if(k<step):
                timeid=k
                k=k+1
            elif(k==step):
        ...
        # col1, col2 = st.columns(2, gap="small")
        # with col1:
            # decomposition_type = st.selectbox("Select Decomposition Type", ["Additive", "Multiplicative"])
        # with col2:            
                
            # if st.button("Run Decomposition"):
                
                # decomposition = seasonal_decompose(data[value_column], model=decomposition_type, period=data_period)  
        ...
        with col11:
            ...
            #st.write("Highest Sale:", top_sale)
        with col22:
            # Histogram
    ...
    if st.session_state['cleaned_data'] is not None:
        ...
        if(len(value_columns)==0):
            ...
            with col1:
                # Forecast settings
                if(time_freq=='MS'):
                    forecast_periods = st.slider("Select Forecast Periods", 1, 6, 3)
                elif(time_freq=='D'):
                    forecast_periods = st.slider("Select Forecast Periods", 1, 30,15)
                else:
                    forecast_periods = st.slider("Select Forecast Periods", 1, 12, 6)            
            with col2:
                model_type1 = st.multiselect("Statistics Model", 
                ["Holt-Winters", "ARIMA", "SARIMA"],default=["Holt-Winters"])
            with col3:
            ...
            with col5:
                model_type4 = st.multiselect("Transformer Model",["Lagllama"])
            ...
            with col2:
                model_type2 = st.multiselect("ML Model", 
                ["Random Forest","XGboost Regression","Voting Regression"],default=["Random Forest"])
            with col3:
        ...
        def run_model():
            with st.spinner(f'Executing the model {model_type}, please wait...'):
                ...
                if "ARIMA" in model_type:
                    ...
                    #model_fit_all_results.append(model_fit)
                ...
                return forecasted_all_results, fitted_data_all_results, model_selected ,lag_mae,lag_mape,lag_rmse
        if st.session_state.models is not None:
            if model_type == st.session_state.models:
                forecasted_all_results, fitted_data_all_results, model_selected,lag_mae,lag_mape,lag_rmse = st.session_state.fcr, st.session_state.fdr, st.session_state.ms,st.session_state.lmae,st.session_state.lmape,st.session_state.lrmse

                pass
            else:
                st.session_state.models = model_type
                st.session_state.fcr, st.session_state.fdr, st.session_state.ms,st.session_state.lmae,st.session_state.lmape,st.session_state.lrmse = run_model()
                forecasted_all_results, fitted_data_all_results, model_selected, lag_mae,lag_mape,lag_rmse= st.session_state.fcr, st.session_state.fdr, st.session_state.ms,st.session_state.lmae,st.session_state.lmape,st.session_state.lrmse
        ...
        with col2:
            ...
            #save_result_db(final_result_df)  
...
with menu[5]:
    ...
    # Use Streamlit's `components.iframe` to display the dashboard
    #st.components.v1.iframe(power_bi_url, width=1080, height=720, scrolling=True)
    iframe_html = f"""
    <iframe 
        src="{power_bi_url}" 
